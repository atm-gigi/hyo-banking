<script setup>
  ...
  const handleDepositClick = () =>
    router.push({ name: 'payment-method', query: { task: TASK_TYPES.DEPOSIT } });
  const handleWithdrawalClick = () =>
    router.push({ name: 'payment-method', query: { task: TASK_TYPES.WITHDRAW } });
...
<template>
  <main class="relative h-screen bg-gray-100 flex flex-col overflow-hidden">
    <!-- 인트로 -->
    <div
      class="absolute inset-0 bg-gray-100 flex flex-col justify-center items-center transition-opacity duration-1000 ease-in-out"
      :class="{ 'opacity-0 pointer-events-none': !isShowAssistantView }"
    >
    ...
    <!-- 거래 선택 -->
    <div
      class="relative w-full h-full flex flex-row justify-between opacity-0 transition-opacity duration-1000 ease-in-out z-10"
