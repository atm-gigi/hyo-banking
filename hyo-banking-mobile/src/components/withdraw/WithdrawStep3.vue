<script setup>
  ...
  const toggleMacroNameInput = () => {
    ...
    if (!props.formData.showMacroNameInput && props.formData.amount) {
      emit('update:formData', {
        macroName: `${props.formData.amount}만원 출금`,
      });
    }
  };
  ...
  <div class="flex-1 flex flex-col">
    <div class="flex-1">
      ...
      <div class="bg-gray-50 rounded-lg p-4 mb-6">
        <h3 class="text-lg font-semibold mb-3">출금 정보 요약</h3>
        <div class="space-y-2 text-sm">
          <div class="flex justify-between">
            <span class="text-gray-600">은행:</span>
            <span class="font-medium">{{
              getBankNameByCode(formData.selectedAccount?.bankCode) ||
              formData.selectedAccount?.bankName
            }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-600">계좌번호:</span>
            <span class="font-medium">{{ formData.selectedAccount?.accountNo }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-600">출금 금액:</span>
